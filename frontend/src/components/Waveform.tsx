...
/**
 * 라이브 막대 파형.
 * - 250ms마다 analyser 진폭(RMS)을 히스토리에 push, 파란 세로 막대로 렌더
 * - marks(북마크)는 파형 위 핀(🔖 + 시간 라벨 칩)으로 표시
 * - active=false(일시정지)면 샘플링을 멈추고 페이드 처리 (스타일은 RecordPage.css)
 */
export function Waveform({ analyser, active, marks, elapsedSec }: WaveformProps) {
  ...
  const draw = useCallback(() => {
    ...
    const capacity = Math.max(2, Math.floor(width / SLOT))
    const visible = historyRef.current.slice(-capacity)

    // 파란 세로 막대
    ctx.fillStyle = '#2563eb'
    for (let i = 0; i < visible.length; i++) {
      const h = Math.max(3, visible[i].amp * (CANVAS_HEIGHT - 16))
      const x = i * SLOT
      const y = mid - h / 2
      ctx.beginPath()
    ...
    if (visible.length >= 2) {
      ...
      for (const m of marksRef.current) {
        ...
        const raw = ((m.timeSec - t0) / span) * innerWidth + BAR_WIDTH / 2
